...
class PairGridCreatorTest {
    ...
    void buildInitialConfig() {
        int rows = 10, cols=10, maxState = 8, numSelected = 3, lives = 0;
        int[] counts = new int[maxState+1];
        PairGridCreator pgc = new PairGridCreator(rows, cols, maxState, numSelected, lives);
        assert(pgc.initialConfig.length==rows && pgc.initialConfig[0].length==cols);
